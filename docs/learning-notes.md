...
## 76. 聊天过程应表达状态，Trace 才负责解释过程（2026-08-24）

把每次工具调用都用圆点、竖线和完成标记塞进 Assistant 消息，会让聊天正文看起来像调试面板，
用户还会误以为这些过程条目属于最终回复。Vesta 的主消息流现在只表达“正在思考”，流式正文
出现后直接承接回复；完整工具步骤、审批状态和验证证据仍保存在 Run Detail 与 Trace 中。

审批界面则需要相反的信息策略：不能只显示内部工具名，而要优先说明“准备做什么、具体内容是
什么、允许后会发生什么”。终端审批直接突出即将执行的命令，Computer 浮窗使用中文动作名称和
诚实状态，原始参数与错误进入折叠技术详情。这样既降低普通用户的理解成本，也没有牺牲诊断
能力。错误提示同样先给出可恢复建议，再按需展示底层错误，而不是把异常类名直接扔给用户。
